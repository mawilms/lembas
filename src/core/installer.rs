...
pub fn install(plugin_id: &str, name: &str) -> Result<(), Box<dyn Error>> {
    let plugin_path = get_plugin_folder();
    let target = format!(
    ...
    let path = plugin_path.join(filename);
    ...
    zip_operation(path.as_path(), &plugin_path);

    Ok(())
}

fn zip_operation(path: &Path, plugin_path: &PathBuf) {
    ...
    zip::ZipArchive::extract(&mut zip_archive, plugin_path).expect("Couldn't extract plugin");
}
