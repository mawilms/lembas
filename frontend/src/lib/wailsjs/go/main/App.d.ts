// Cynhyrchwyd y ffeil hon yn awtomatig. PEIDIWCH Â MODIWL
// This file is automatically generated. DO NOT EDIT
import {models} from '../models';

export function DeletePlugin(arg1:string,arg2:string):Promise<Array<models.LocalPluginModel>>;

export function FetchRemotePlugins():Promise<Array<models.RemotePluginModel>>;

export function GetInstalledPlugins():Promise<Array<models.LocalPluginModel>>;

export function InstallPlugin(arg1:string):Promise<Array<models.LocalPluginModel>>;

export function SaveSettings(arg1:string):Promise<void>;

export function UpdatePlugins(arg1:any):Promise<void>;
